...
def restructure_shapes(
    shape_lines: list, channels: list, samples: list, systematics: list, mass: str
) -> list:
...
def restructure_observations(observations: list, shapes_list: list) -> list:
    ...
    for i_ch, channel in enumerate(observations[0].split()[1:]):
        ...
        for shape_dict in shapes_list:
            if (
                shape_dict["sample"] == "data_obs"
                and shape_dict["channel"] == channel
...
def restructure_channels(samples: list) -> list:
    ...
    for ch in sorted(set(channel_names)):
        ...
        sample_dict_list = []
        for i_sam, (sample, sample_num) in enumerate(
            zip([sample_names[i] for i in ch_idx], [samples_numbers[i] for i in ch_idx])
        ):
            ...
            if syst_type == "lnN":
                ...
                for shape_identifier in shape_identifiers:
                    if (
                        ...
                        and shape_identifier["sample"] == sample_name
                        and shape_identifier["channel"] == channel_name
...
def get_sections_dict(datacard: list) -> dict:
    ...
    if "shapes" in sections_dict:
        sections_dict["shapes"] = restructure_shapes(
            sections_dict["shapes"], channel_names, sample_names, modifier_names, mass
        )
    else:
            ...
            if sections_dict["shapes"]:
                ...
                for shape_dict in sections_dict["shapes"]:
                    if (
                        ...
                        and shape_dict["sample"] == sample["name"]
